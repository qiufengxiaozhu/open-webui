<script>
	...
	const executePythonAsWorker = async (id, _code, cb, _files = []) => {
		const response = {
			stdout: null,
			stderr: 'Python execution is not available on this platform',
			result: null
		};

		if (cb) {
			cb(JSON.parse(JSON.stringify(response)));
		}
	};

	const resolveToolServer = (serverUrl) => {
		const toolServer = $settings?.toolServers?.find((server) => server.url === serverUrl);
		const toolServerData = $toolServers?.find((server) => server.url === serverUrl);
