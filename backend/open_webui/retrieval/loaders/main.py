...
def _require_langchain_loader(loader, name: str):
    if loader is None:
        raise ValueError(
            f"LangChain loader '{name}' is not available. Install langchain-community to use this engine."
        )
    return loader
...
class Loader:
    ...
    def _get_loader(self, filename: str, file_content_type: str, file_path: str):
        ...
        if (
            ...
            if ExternalDocumentLoader is None:
                raise ValueError(
                    "External document loader engine is not available. "
                    'Install the external document loader module to use this engine.'
                )
            ...
            if self._is_text_file(file_ext, file_content_type):
                loader = _require_langchain_loader(TextLoader, 'TextLoader')(
            ...
            if DatalabMarkerLoader is None:
                raise ValueError(
                    "Datalab Marker document loader engine is not available. "
                    'Install the datalab_marker loader module to use this engine.'
                )
            ...
            AzureLoader = _require_langchain_loader(
            ...
            if self.kwargs.get('DOCUMENT_INTELLIGENCE_KEY') != '':
                ...
                if DefaultAzureCredential is None:
                    raise ValueError(
                        "Azure Document Intelligence engine requires azure-identity. "
                        'Install azure-identity or provide DOCUMENT_INTELLIGENCE_KEY.'
                    )
                loader = AzureLoader(
                    file_path=file_path,
                    api_endpoint=self.kwargs.get('DOCUMENT_INTELLIGENCE_ENDPOINT'),
            ...
            if MinerULoader is None:
                raise ValueError(
                    "MinerU document loader engine is not available. "
                    'Install the mineru loader module to use this engine.'
                )
            ...
            if MistralLoader is None:
                raise ValueError(
                    "Mistral OCR document loader engine is not available. "
                    'Install the mistral loader module to use this engine.'
                )
            ...
            if PaddleOCRVLLoader is None:
                raise ValueError(
                    "PaddleOCR-VL document loader engine is not available. "
                    'Install the paddleocr_vl loader module to use this engine.'
                )
            ...
            if file_ext == 'pdf':
                loader = _require_langchain_loader(PyPDFLoader, 'PyPDFLoader')(
                    file_path,
                    extract_images=self.kwargs.get('PDF_EXTRACT_IMAGES'),
                    mode=self.kwargs.get('PDF_LOADER_MODE', 'page'),
                )
            elif file_ext == 'csv':
                loader = _require_langchain_loader(CSVLoader, 'CSVLoader')(
                    file_path, autodetect_encoding=True
                )
            elif file_ext == 'rst':
                try:
                ...
                loader = _require_langchain_loader(BSHTMLLoader, 'BSHTMLLoader')(
                ...
                loader = _require_langchain_loader(Docx2txtLoader, 'Docx2txtLoader')(file_path)
                ...
                loader = _require_langchain_loader(OutlookMessageLoader, 'OutlookMessageLoader')(file_path)
            elif file_ext == 'odt':
                try:
