...
class SkillsGate:
    ...
    def build_full_system_prompt(self) -> str:
        ...
        return (
            ...
            '## 输出格式\n'
            '请输出结构化 Markdown 报告（**不要包含修复建议和预防措施**）：\n'
            '- 故障概况（含 taskId、文件类型、错误码）\n'
            '- 关键日志证据（每条日志必须注明：`文件名:行号`，行号**必须来自工具返回的 line_number 字段**）\n'
            '- 根因分析（三层根因，每层引用具体日志证据及其 `文件名:行号`）\n'
            '- 时间线（**仅包含该 taskId 在连续时间段内的事件**，每条标注 `来源文件:行号`）\n'
            '- 影响范围\n\n'
            '**禁止输出的章节**：修复建议、排查路径、预防措施、解决方案。\n'
            '报告只做故障诊断和根因定位，不做修复指导。\n\n'
            '### 日志引用示例（必须遵循此格式）\n'
            '```\n'
            '**来源**: `TaskServer_2/error-2026-05-25.log:1523`（行号来自 grep_log 返回）\n'
            '```\n'
            '上面的 1523 必须是 grep_log 或 get_context 工具返回中的 line_number 值，\n'
            '**不是你自己估算的行号**。'
        )
