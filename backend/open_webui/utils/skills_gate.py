...
class SkillsGate:
    ...
    def build_full_system_prompt(self) -> str:
        ...
        return (
            ...
            '脚本位于项目 docs/skills/scripts/ 目录下，分析日志时优先通过 run_script 工具调用\n'
            '对应的分析脚本快速定位问题，再结合技能文档中的链路知识做深入分析。\n'
            '调用示例（lang="bash"）：\n'
            '```\n'
            'python3 docs/skills/scripts/analyze_openapi_failure.py --logDir <日志目录> --taskId <taskId>\n'
            'python3 docs/skills/scripts/analyze_task_failure.py --logDir <日志目录> --docId <docId>\n'
            '```\n\n'
            '## 工作流程\n'
            '1. 先判断问题是否在支持领域内\n'
            '2. 向用户确认必要信息（时间、taskId/docId、日志目录路径）\n'
            '3. 优先调用对应技能的分析脚本做初步扫描\n'
            '4. 根据脚本输出的阶段和错误码，结合技能文档中的链路知识深入分析\n'
            '5. 证据不足时使用 grep_log 等工具补充搜索\n'
            '6. 最终输出结构化诊断报告\n\n'
            '## 输出格式\n'
            '请输出结构化 Markdown 报告：\n'
            '- 故障概况\n'
