...
| 1203 | MIME/格式校验失败 | 文件扩展名与实际内容格式不匹配，或文件损坏/为空导致无法识别 MIME 类型。detail.id=415 表示 `Invalid File Mime Type`，detail.id=529 表示 `Corrupted File`。常见原因：文件被错误命名（如文本改名为 .docx）、文件内容为空、业务系统下载接口返回了 HTML 错误页面而非真实文件 |
| 1214 | 转换失败（通用） | 转换过程中发生不可恢复错误的通用码，可能原因很多（OOM、引擎崩溃、PDF合并失败、格式不支持等），**不要将 1214 绑定到某个特定异常类型**，必须结合日志中的具体错误信息判断真实原因 |
